...
module FormMinKata =
    ...
    let FormMin(arr : int list) : int =
        ...
        let concatState =
            Array.fold accumulateDigit { PlaceValueIndex = 0
                                         Accumulator = 0 } uniqueDigits
...
module SimpleEncryptionAlternatingSplitKata =
    ...
    let encryptOnce (text : string) : string =
        ...
        let everyIndexedSecondChar, everyIndexedOtherChar =
            indexedChars |> Array.partition (fun (index, _) -> index % 2 = 1)
    ...
    let decryptOnce (text : string) : string =
        ...
        let decryptedChars =
            if everySecondChar.Length = everyOtherChar.Length then Array.zip everyOtherChar everySecondChar |> flatten
            else
                let lastChar = everyOtherChar.[everyOtherChar.Length - 1]
                let butLastEveryOtherChar = everyOtherChar.[0..everyOtherChar.Length - 2]
                Array.zip butLastEveryOtherChar everySecondChar |> flattenAndAppendExtra lastChar
    ...
    let processTextWith (f : string -> string) (text : string) (nTimes : int) : string =
        if not (String.IsNullOrEmpty text || nTimes <= 0) then
            seq { 1..nTimes } |> Seq.fold (fun textBlock _ -> f textBlock) text
        else text

    let encrypt (text : string) (n : int) : string = processTextWith encryptOnce text n
    let decrypt (encryptedText : string) (n : int) : string = processTextWith decryptOnce encryptedText n
...
module TribonacciKata =
    let tribonacci (signature : int list) (n : int) : int list =
        if n <= 3 then signature |> List.take n
        else
            let startTriple =
            ...
            let nextTrib = function
                | (a, b, c) -> Some(a + b + c, (b, c, a + b + c))
            signature @ (startTriple
                         |> Seq.unfold nextTrib
                         |> Seq.take (n - 3)
                         |> List.ofSeq)
...
module RGBToHex =
    ...
    let rgb (r : int) (g : int) (b : int) : string =
        ...
        let hex (n : int) : string = sprintf "%02X" (normalise n) // X=hex, 0 means zero pad, 2 padding length
        (hex r) + (hex g) + (hex b)
...
module PIApproxKata =
    ...
    let iterPi (epsilon : double) : int * double =
        ...
        let rec nextIteration iterations piApproximation seriesSum termDenominator isAddition =
            if Math.Abs(piApproximation - PI) < epsilon then (iterations, piApproximation)
            else
                let denominator = termDenominator + 2
                let termValue = 1.0 / double (denominator)

                let nextSeriesSum =
                    if isAddition then seriesSum + termValue
                    else seriesSum - termValue
...
module WhichAreInKata =
    let inArray (a1 : string list) (a2 : string list) =
        let contains (s1 : string) (s2 : string) = s2.Contains(s1)
        [ for s in a1 do
...
module CalcStringRotation =
    ...
    let ShiftedDiff (first : string) (second : string) : int =
        // Should really have check that the lengths are equal
        seq {
            for i in 0..first.Length -> (i, rotateString i first)
        }
